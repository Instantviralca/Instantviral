export { HeroSection } from '@/components/marketing/hero/hero-section';
export { HeroContent } from '@/components/marketing/hero/hero-content';
export { HeroActions } from '@/components/marketing/hero/hero-actions';
export { HeroTrustBar } from '@/components/marketing/hero/hero-trust-bar';
export { HeroVisual } from '@/components/marketing/hero/hero-visual';
export { PlatformBadge } from '@/components/marketing/hero/platform-badge';

/** Backward-compatible alias used by HomePageView / marketing barrel. */
export { HeroSection as Hero } from '@/components/marketing/hero/hero-section';

export type {
  HeroSectionProps,
  HeroSectionProps as HeroProps,
  HeroContentProps,
  HeroActionsProps,
  HeroTrustBarProps,
  HeroVisualProps,
  PlatformBadgeProps,
} from '@/components/marketing/hero/types';
