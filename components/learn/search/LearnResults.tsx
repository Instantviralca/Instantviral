...
import { ArticleCardMedia } from '@/components/learn/ArticleCardMedia';
  ...
  return (
    ...
    >
      {items.map((item) => {
        ...
        return (
          <li key={item.id}>
            <article className="group flex h-full flex-col overflow-hidden border border-neutral-200 bg-white transition-colors hover:border-neutral-400">
              <Link
                href={item.href}
                className="block rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-neutral-900 focus-visible:ring-offset-2"
                onClick={() => onResultClick?.(item)}
                aria-label={`Read ${item.title}`}
              >
                <ArticleCardMedia image={item.featuredImage} title={item.title} />
              </Link>
